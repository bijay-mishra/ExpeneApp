...
import { endOfMonth, endOfWeek, format, isValid, isWithinInterval, parseISO, startOfMonth, startOfWeek, subMonths, subWeeks } from 'date-fns';
...
const YAxis = ({ data }) => {
    ...
    return (
        <View style={styles.yAxisContainer}>
            ...
            <Text style={styles.yAxisLabel}>{currency.symbol}0</Text>
        </View>
    );
};
    ...
    const chartData = useMemo(() => {
        ...
        if (timePeriod === 'weekly') {
            ...
            for (let i = 0; i < 4; i++) {
                ...
                const weeklyTx = transactions.filter(t => isValid(parseISO(t.date)) && isWithinInterval(parseISO(t.date), { start: weekStart, end: weekEnd }));
            ...
            for (let i = 0; i < 4; i++) {
                ...
                const monthlyTx = transactions.filter(t => isValid(parseISO(t.date)) && isWithinInterval(parseISO(t.date), { start: monthStart, end: monthEnd }));
        ...
        <SafeAreaView style={styles.container}>
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
                <View style={styles.summaryContainer}>
                    ...
                    <View style={[styles.summaryBox, styles.expenseSummaryBox]}>
